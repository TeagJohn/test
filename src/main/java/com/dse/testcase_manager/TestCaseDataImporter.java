...
//                                    String str = element.getAsString().substring(1, element.getAsString().length()-1);
                        ...
                        if (jsonObject.get(TemplateSubprogramDataNode.LIST_ARG_TEMPLATE) != null) {
                            JsonArray listArgTemplateJson = jsonObject.getAsJsonArray(TemplateSubprogramDataNode.LIST_ARG_TEMPLATE);
                            List<ValueDataNode> listArgTemplate = new ArrayList<>();
                            for (JsonElement element : listArgTemplateJson.getAsJsonArray()) {
                                //todo: do something here
                                IValueDataNode valueDataNode = context.deserialize(element, DataNode.class);
                                ((TemplateSubprogramDataNode) node).getListArgOfTemplate().add(valueDataNode);
//                                listArgTemplate.add(element);
                            }
//                            ((TemplateSubprogramDataNode) dataNode).setRealTypeMapping(realTypeMapping);
                        }

                        if (jsonObject.get(TemplateSubprogramDataNode.REAL_FUNC) != null) {
                            String def = jsonObject.get(TemplateSubprogramDataNode.REAL_FUNC).getAsString();
                            ((TemplateSubprogramDataNode) node).setRealFunction(def);
                        }
...
//                        }
    ...
    private boolean loadCorrespondingDependency(JsonObject jsonObject, ValueDataNode dataNode) {
        if (jsonObject.get("correspondingVar") != null && !jsonObject.get("correspondingVar").getAsString().equals("")) {
    ...
    private boolean loadVariableFromCorrespondingType(ValueDataNode dataNode,
        ...
        if (type == null) {
            if (dataNode instanceof NormalDataNode
                    ...
                    || dataNode instanceof PointerDataNode || dataNode instanceof PointerCharacterDataNode
