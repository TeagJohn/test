...
//
                ...
                INode functionNode = ((SubprogramNode) node).getFunctionNode();
                if (functionNode != null && functionNode.getAbsolutePath() != null) {
                    String path = PathUtils.toRelative(functionNode.getAbsolutePath());
                    json.addProperty("functionNode", path);
                }
                ...
                if (node instanceof TemplateSubprogramDataNode) {
                    ...
                    //add list arg of template function to json
                    JsonArray listArgOfTemplateJson = new JsonArray();
                    for (IValueDataNode arg : ((TemplateSubprogramDataNode) node).getListArgOfTemplate()) {
                        listArgOfTemplateJson.add(jsonSerializationContext.serialize(arg, DataNode.class));
                    }
                    json.add(TemplateSubprogramDataNode.LIST_ARG_TEMPLATE, listArgOfTemplateJson);

                    //add real function of template function to json
                    DefinitionFunctionNode definition = ((TemplateSubprogramDataNode) node).getRealFunctionNode();
                    json.addProperty(TemplateSubprogramDataNode.REAL_FUNC, (definition == null) ? "" : definition.toString());
                ...
                IVariableNode correspondingVar = dataNode.getCorrespondingVar();
                String varRelativePath = (correspondingVar == null) ? "" : PathUtils.toRelative(correspondingVar.getAbsolutePath());
...
//                if (!((ClassDataNode) node).isUseUserCode()) {
                ...
                String varRelativePath = (dataNode.getCorrespondingVar() == null) ? "" : PathUtils.toRelative(dataNode.getCorrespondingVar().getAbsolutePath());
                json.addProperty("correspondingVar", varRelativePath);

                INode correspondingType = dataNode.getCorrespondingType();
                String typeRelativePath = (correspondingType == null) ? "" : PathUtils.toRelative(correspondingType.getAbsolutePath());
                json.addProperty("correspondingType", typeRelativePath);
//                }
