...
public class InstructionComputator {
    ...
    public static InstructionMapping nStatements = new InstructionMapping();
    public static InstructionMapping nBranches = new InstructionMapping();
    public static InstructionMapping nMcdcs = new InstructionMapping();
    public static InstructionMapping nBasicPaths = new InstructionMapping();
    public static List<String> computedSourcecodes = new ArrayList<>();

    public static void compute(INode consideredSourcecodeNode) {

        logger.debug("Compute the number of statements/branches and saved in file");
        if (consideredSourcecodeNode instanceof ISourcecodeFileNode &&
                // to avoid duplicating instruction computation
                !computedSourcecodes.contains(consideredSourcecodeNode.getAbsolutePath())) {
            computedSourcecodes.add(consideredSourcecodeNode.getAbsolutePath());

            try {
                InstructionComputator instructionComputator = new InstructionComputator();
                int[] instructionComputation = instructionComputator.getNumberOfBranchesAndStatements(consideredSourcecodeNode);

                int nStatement = instructionComputation[InstructionComputator.STATEMENT_COV_INDEX];
                nStatements.put(PathUtils.toRelative(consideredSourcecodeNode.getAbsolutePath()), nStatement);

                int nBranch = instructionComputation[InstructionComputator.BRANCH_COV_INDEX];
                nBranches.put(PathUtils.toRelative(consideredSourcecodeNode.getAbsolutePath()), nBranch);

                int nMcdc = instructionComputation[InstructionComputator.MCDC_COV_INDEX];
                nMcdcs.put(PathUtils.toRelative(consideredSourcecodeNode.getAbsolutePath()), nMcdc);

                int nBasicPath = instructionComputation[InstructionComputator.BASIC_PATH_COV_INDEX];
                nBasicPaths.put(PathUtils.toRelative(consideredSourcecodeNode.getAbsolutePath()), nBasicPath);
            } catch (Exception e) {
