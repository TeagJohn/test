...
//                    Utils.writeContentToFile(newContent, cloneFilePath);
        ...
        for (ISourcecodeFileNode sourceCode : sources) {
            cloneSourceCodeTasks.add(() -> {
                cloneASourceCodeFile(sourceCode);
                return "Done";
            });
        }
                    ...
                    for (String libraryName : libraries.keySet()) {
                        ...
                        if (SystemLibrary.isUseLibrary(libraryName, function)) {
                            ...
                            newFunctionCode = newFunctionCode
                                    .replaceAll(
                                            "\\b" + Pattern.quote(VariableTypeUtils.STD_SCOPE + libraryName)
                                                    + "\\s*?\\(",
                                            newLibraryCall + "(");

                            newFunctionCode = newFunctionCode
                                    .replaceAll("\\b" + Pattern.quote(libraryName) + "\\s*?\\(", newLibraryCall + "(");
    ...
    public static void compileIgnoreFiles(String directory, ITestCase testCase) {
        ...
        for (Map.Entry<String, String> entry : rootCommand.getCompilationCommands().entrySet()) {
            ...
            try {
                ...
                // UIController.showDetailDialog(Alert.AlertType.ERROR, "Compilation error",
                // "Unable to compile " + newPath, ex.getMessage());
                logger.error("Error when compiling " + newPath + ":\n" + ex.getMessage());
            }
