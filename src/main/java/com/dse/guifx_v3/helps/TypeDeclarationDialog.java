...
public abstract class TypeDeclarationDialog extends Alert {
    ...
    private void generateTableView() {
        ...
        TableColumn<String, String> typeCol = new TableColumn<>("Type/Value");
        ...
        if (functionNode.isTemplate() && functionNode instanceof IFunctionNode) {
            ...
            templateDeclaration = (CPPASTTemplateDeclaration) ((IFunctionNode) functionNode).getAST().getParent();
...
//                tableView.getItems().add(templateParam);
            for (ICPPASTTemplateParameter parameter : templateDeclaration.getTemplateParameters()) {
                ...
                IASTName name = null;
                if (parameter instanceof CPPASTSimpleTypeTemplateParameter) {
                    name = ((CPPASTSimpleTypeTemplateParameter) parameter).getName();
                }
                else if (parameter instanceof CPPASTParameterDeclaration) {
                    name = ((CPPASTParameterDeclaration) parameter).getDeclarator().getName();
                }
                if (name != null) {
                    tableView.getItems().add(name.getRawSignature());
                }
            }
        }
