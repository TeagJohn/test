...
import static com.dse.config.WorkspaceConfig.ENV_EXTENSION;
...
//    @FXML
//    public CustomMenuItem cmiViewCoverage;

    @FXML
    private CustomMenuItem cmiViewTestCaseManage;

    @FXML
    private CustomMenuItem cmiViewTestCaseData;

    @FXML
    private CustomMenuItem cmiViewFull;

    @FXML
    private Menu mView;

    @FXML
    private CustomMenuItem cmiViewEnvironment;

    @FXML
    private CustomMenuItem cmiAutomatedTestdataGenerationConfig;

    @FXML
    private MenuBar mMenuBar;

//    @FXML
//    private CustomMenuItem cmiCleanAtSourcecodeFileLevel;
//
//    @FXML
//    private CustomMenuItem cmiOpenTextEditorConfig;
    ...
    @FXML
    private Menu mEnvironment;

    @FXML
    private CustomMenuItem cmiStopAutomatedTestdataGeneration;

    @FXML
    private CustomMenuItem cmiDebug;

    @FXML
    private CustomMenuItem cmiClean;

    @FXML
    private CustomMenuItem cmiRunWithReport;

    @FXML
    private CustomMenuItem cmiRunWithoutReport;

    @FXML
    private CustomMenuItem cminewCCPPEnvironment;

    @FXML
    private CustomMenuItem cmiOpenProject;
...
//    MenuItem miSetWorkingDirectory;
    ...
    @FXML
    private CustomMenuItem cmiSetupBoundOfVariableType;

    @FXML
    private CustomMenuItem cmiUserCode;

    private BackgroundTaskObjectController rebuildEnvironmentBGController;

//    @FXML
//    private CustomMenuItem cmiImportTestcases;
    ...
    @FXML
    private CustomMenuItem cmiUserManual;

    @FXML
    private Label labelHelp;

    @FXML
    private CustomMenuItem cmiToolVersion;

    @FXML
    private CustomMenuItem cmiLicense;

//    @FXML
//    private CustomMenuItem cmiEtc;


    //Wednesday, April 14th, 2021

    @FXML
    private CustomMenuItem cmiAnalyzeDependencies;

    @FXML
    private CustomMenuItem cmiCleanAtSourcecodeFileLevel;

    @FXML
    private CustomMenuItem cmiIncrementalBuild;

    @FXML
    private CustomMenuItem cmiInstrumentProject;

    @FXML
    private CustomMenuItem cmiShowAvailableRegressionScripts;

    @FXML
    private CustomMenuItem cmiUpdateEnvironment;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        mEnvironment.disableProperty().addListener(new ChangeListener<Boolean>() {
            @Override
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                logger.debug("Environment Menu disability change " + oldValue + " to " + newValue);
            }
        });
        mEdit.disableProperty().addListener(new ChangeListener<Boolean>() {
            @Override
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                logger.debug("Edit Menu disability change " + oldValue + " to " + newValue);
            }
        });
        mRun.disableProperty().addListener(new ChangeListener<Boolean>() {
            @Override
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                logger.debug("Run Menu disability change " + oldValue + " to " + newValue);
            }
        });
        mSetting.disableProperty().addListener(new ChangeListener<Boolean>() {
            @Override
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                logger.debug("Setting Menu disability change " + oldValue + " to " + newValue);
            }
        });
        mView.disableProperty().addListener(new ChangeListener<Boolean>() {
            @Override
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                logger.debug("View Menu disability change " + oldValue + " to " + newValue);
            }
        });
        ...
        Hint.tooltipNode(cmiUserManual.getContent(), HintContent.MenuBar.USER_MANUAL_HINT);
//        Hint.tooltipNode(cmiToolVersion.getContent(), HintContent.MenuBar.TOOL_VERSION_HINT);
        ...
        Hint.tooltipNode(cmiIncrementalBuild.getContent(), HintContent.MenuBar.INCREMENTAL_BUILDING_HINT);
        Hint.tooltipNode(cmiUpdateEnvironment.getContent(), HintContent.MenuBar.UPDATE_ENV_HINT);
        Hint.tooltipNode(cmiAnalyzeDependencies.getContent(), HintContent.MenuBar.ANALYSIS_DEPENDENCIES_HINT);
        Hint.tooltipNode(cmiInstrumentProject.getContent(), HintContent.MenuBar.INSTRUMENT_HINT);
        Hint.tooltipNode(cmiShowAvailableRegressionScripts.getContent(), HintContent.MenuBar.REGRESSION_HINT);
        Hint.tooltipNode(cmiClean.getContent(), HintContent.MenuBar.CLEAN_FUNCTION_HINT);
        Hint.tooltipNode(cmiCleanAtSourcecodeFileLevel.getContent(), HintContent.MenuBar.CLEAN_FILE_HINT);
        Hint.tooltipNode(cmiUserCode.getContent(), HintContent.MenuBar.USER_CODE_HINT);
        Hint.tooltipNode(cmiRunWithoutReport.getContent(), HintContent.MenuBar.RUN_WITHOUT_REPORT_HINT);
        Hint.tooltipNode(cmiRunWithReport.getContent(), HintContent.MenuBar.RUN_HINT);
        Hint.tooltipNode(cmiDebug.getContent(), HintContent.MenuBar.DEBUG_HINT);
        Hint.tooltipNode(cmiStopAutomatedTestdataGeneration.getContent(), HintContent.MenuBar.STOP_AUTOGEN_HINT);
        Hint.tooltipNode(cmiAutomatedTestdataGenerationConfig.getContent(), HintContent.MenuBar.SET_AUTOCONFIG_HINT);
        Hint.tooltipNode(cmiSetupBoundOfVariableType.getContent(), HintContent.MenuBar.SET_BOUND_HINT);
        Hint.tooltipNode(cmiViewEnvironment.getContent(), HintContent.MenuBar.VIEW_ENV_HINT);
        Hint.tooltipNode(cmiViewTestCaseData.getContent(), HintContent.MenuBar.VIEW_TEST_DATA_HINT);
        Hint.tooltipNode(cmiViewFull.getContent(), HintContent.MenuBar.VIEW_FULL_HINT);
        Hint.tooltipNode(cmiViewTestCaseManage.getContent(), HintContent.MenuBar.VIEW_TEST_MANAGE_HINT);
...
//        }
    ...
    private boolean isOpeningWorkspace(File environmentFile) {
        ...
        String workspace = environmentFilePath.replace(ENV_EXTENSION, SpecialCharacter.EMPTY);
...
//            }
    ...
    private void setUpWorkingDirectoryAgain(File environmentFile) {
        ...
        CMakeBuilder.resetExecutableFilePath();
...
//        logger.debug("is open environment = " + !openedProject);
...
//            mEdit.setDisable(openedProject);
        cmiClean.setDisable(openedProject || !isActiveMultiSelection);
        cmiCleanAtSourcecodeFileLevel.setDisable(openedProject || !isActiveMultiSelection);
...
//            mRun.setDisable(openedProject);
        cmiRunWithoutReport.setDisable(openedProject || !isActiveMultiSelection);
        cmiRunWithReport.setDisable(openedProject || !isActiveMultiSelection);
        cmiDebug.setDisable(openedProject || !isActiveMultiSelection);
        cmiStopAutomatedTestdataGeneration.setDisable(openedProject);
...
//        miStopAutomatedTestdataGeneration.setDisable(!AkaThreadManager.runningAtLeastOneAutomatedTestdataGenerationThread());
        ...
        // SETTING
//        if (mSetting.isDisable())
//            mSetting.setDisable(openedProject);
        cmiAutomatedTestdataGenerationConfig.setDisable(openedProject);
        cmiSetupBoundOfVariableType.setDisable(openedProject);
...
//            mView.setDisable(openedProject);
        cmiViewEnvironment.setDisable(openedProject);
        cmiViewFull.setDisable(openedProject || !isActiveMultiSelection);
        cmiViewTestCaseData.setDisable(openedProject || !isActiveMultiSelection);
        cmiViewTestCaseManage.setDisable(openedProject || !isActiveMultiSelection);
    }
    ...
    public void instrumentProject(ActionEvent actionEvent) {
        ...
        AbstractAkaTask<Void> task;
//        task = new AbstractAkaTask<Void>() {
//            @Override
//            protected Void call() throws Exception {
//                ProjectClone.cloneEnvironment();
...
//            }
//        };
        task = new ProjectClone.CloneThread();
    ...
    public void clearCache(ActionEvent actionEvent) {
        AbstractAkaTask<Void> task = new AbstractAkaTask<Void>() {
            @Override
            protected Void call() throws Exception {
                List<String> recentEnvironments = new AkaConfig().fromJson().getRecentEnvironments();
                Set<File> deletedWorkingDirs = new HashSet<>();
                for (String recentEnvironment : recentEnvironments) {
                    File workingDir = new File(recentEnvironment).getParentFile();
                    if (workingDir.exists() && !deletedWorkingDirs.contains(workingDir) && workingDir.listFiles() != null) {
                        File[] envDirs = workingDir.listFiles();
                        if (envDirs != null) {
                            for (File openingWsDir : envDirs) {
                                if (openingWsDir.isDirectory()) {
                                    File exeDir = openingWsDir.toPath().resolve("exe").toFile();
                                    File coverageDir = openingWsDir.toPath().resolve("coverages").toFile();
                                    File reportDir = openingWsDir.toPath().resolve("reports").toFile();
                                    File testDriverDir = openingWsDir.toPath().resolve("test-drivers").toFile();
                                    File[] dirs = new File[]{exeDir, coverageDir, reportDir, testDriverDir};
                                    for (File dir : dirs)
                                        deleteFiles(dir);
                                    logger.debug("Clear cache in " + openingWsDir);
                                }
                            }
                        }
                        deletedWorkingDirs.add(workingDir);
                    }
                }
                return null;
            }
        ...
        task.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent event) {
                UIController.showSuccessDialog("Clear cache successfully",
                        "Clear cache", "Success");
            }
        });
    }
