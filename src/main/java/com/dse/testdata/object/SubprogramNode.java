    ...
    public void initInputToExpectedOutputMapWithProtoType(String testCaseJsonFile) {
        ...
        try {
            if (new File(testCaseJsonFile).exists()) {
                ...
                //get return type
                JsonArray unitNodeParameter = firstChildren.getAsJsonArray("children").get(1).getAsJsonObject().getAsJsonArray("children");
                String retType = "";
                //parameter: choose the 1st param
                String newType = unitNodeParameter.get(0).getAsJsonObject().getAsJsonPrimitive("dataType").getAsString();
                    ...
                    if (child instanceof VariableNode) {
                        ...
                        for (ValueDataNode actual : actualNodes) {
                            if (actual.getName().equals(expected.getName())) {
                                expected.setRawType(expected.getRawType().replace(retType, newType));
                                expected.setRealType(expected.getRealType().replace(retType, newType));
