...
public abstract class AbstractInitialTreeGen implements IInitialSubTreeGen {
    ...
    public static List<String> filterCalledFunctions(ICommonFunctionNode functionNode) {
        ...
        for (IASTSimpleDeclaration d : parser.unexpectedCalledFunctions) {
            if (d.getDeclarators().length == 1) {
                IASTDeclarator dtor = d.getDeclarators()[0];
                if (dtor.getName().getRawSignature().isEmpty() && dtor.getNestedDeclarator() != null) {
                    String func = d.getDeclSpecifier().getRawSignature();
                    calledFunctionsName.add(func);
                }
            }
        }
