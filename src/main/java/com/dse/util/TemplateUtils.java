...
public class TemplateUtils {
    private final static AkaLogger logger = AkaLogger.get(TemplateUtils.class);
    ...
    public static boolean isTemplateTypeDefinedByUser(String rawType, ICommonFunctionNode fn) {
        if (fn instanceof MacroFunctionNode) return false;
        ...
        if (functionNode.isTemplate()) {
            ...
            if (templateDeclaration != null) {
                ...
                for (ICPPASTTemplateParameter parameter : parameters) {
                    // get typename"T"
                    String typeName = ((ICPPASTTemplateParameter) parameter).getRawSignature();
//                    if (typeName.equals(rawType)){
    ...
    public static List<IVariableNode> getTemplateArgs(IFunctionNode functionNode) {
        IASTNode[] listAST = functionNode.getAST().getParent().getChildren();
        List<IVariableNode> vars = new ArrayList<>();
        for (IASTNode child : listAST) {
            if (child instanceof ICPPASTParameterDeclaration) {
                IVariableNode variableNode = new VariableNode();
                variableNode.setAST(child);
    ...
    public static boolean isTemplateClass(String rawType) {
        ...
        return rawType.contains(OPEN_TEMPLATE_ARG) && rawType.endsWith(CLOSE_TEMPLATE_ARG);
    }

    public static boolean isTemplate(String rawType) {
        return rawType.contains(OPEN_TEMPLATE_ARG) && rawType.contains(CLOSE_TEMPLATE_ARG);
    }
    ...
    public static boolean isChildOfTemplateClass(IVariableNode variableNode) {
        ...
        while (parent != null) {
            if (parent instanceof ClassNode && ((ClassNode) parent).isTemplate()) return true;
    ...
    public static String getTemplateClass(String rawType) {
        ...
        String coreType = rawType.substring(0, index) + rawType.substring(index).replaceAll(IRegex.POINTER, "").replaceAll(IRegex.ADDRESS, "").replaceAll(IRegex.ARRAY_INDEX, "");
    ...
    public static String getTemplateFullRawType(VariableNode variableNode) {
        ...
        List<SearchCondition> conditions = Arrays.asList(new FunctionNodeCondition(), new DefinitionFunctionNodeCondition());

        List<INode> functions = Search.searchNodes(Environment.getInstance().getProjectNode(), conditions);
        ...
        rawType = rawType.replace("<::", "<").replace(", ::", ", ");
    ...
    public static String recursiveGetFullRawType(String rawType, List<Level> space) {
        // Get Core Type
        String coreType = rawType.replaceAll(IRegex.POINTER, SpecialCharacter.EMPTY).replaceAll(IRegex.ARRAY_INDEX, SpecialCharacter.EMPTY);

        if (isTemplate(rawType)) coreType = getTemplateClass(rawType);
    ...
    public static String[] getTemplateParameters(INode node) {
        ...
        if (node instanceof DefinitionFunctionNode) ast = ((DefinitionFunctionNode) node).getAST();
        else if (node instanceof AbstractFunctionNode) ast = ((AbstractFunctionNode) node).getAST();
        else if (node instanceof StructOrClassNode) ast = ((StructOrClassNode) node).getAST();
        ...
        if (astVariable != null) {
            ...
            if (declSpecifier instanceof IASTNamedTypeSpecifier) {
                ...
                if (name instanceof ICPPASTTemplateId) {
                    ...
                    templateArgs = Arrays.stream(templateArgASTNodes).filter(Objects::nonNull).map(IASTNode::getRawSignature).toArray(String[]::new);
                    ...
                    for (IASTFunctionCallExpression expression : visitor.getExpressions()) {
                        ...
                        if (called != null && called.equals(functionNode)) {
                            ...
                            if (isTemplate(name)) {
                                ...
                                if (astName instanceof ICPPASTQualifiedName) astName = astName.getLastName();
                                ...
                                if (astName instanceof ICPPASTTemplateId) {
                                    for (IASTNode arg : ((ICPPASTTemplateId) astName).getTemplateArguments())
                                        if (arg != null) templateArgs.add(arg.getRawSignature());
                                }
                                ...
                                //create prototype here //todo: change your strategy here
                                prototype = "";
                                ...
                                for (int i = 0; i < arguments.length; i++) {
                                    if (arguments[i] instanceof IASTExpression) {
                                        ...
                                        paramType = paramType.replaceAll(" \\*", "*").replaceAll(" \\[", "[");
...
//                                        List<String> indexes = Utils.getIndexOfArray(deleteTemplateParameters(parameters.get(i).getRawType()));
                                        ...
                                        if (indexes.size() > 0) {
                                            ...
                                            while (paramType.charAt(idx) == SpecialCharacter.CLOSE_SQUARE_BRACE || paramType.charAt(idx) == SpecialCharacter.OPEN_SQUARE_BRACE || Character.isDigit(paramType.charAt(idx)))
                                                idx--;
        ...
        if (functionNodes.isEmpty()) {
            DefinitionFunctionNode suggestionInt = getPrimitivePrototype("int", functionNode);
            DefinitionFunctionNode suggestionString = getPrimitivePrototype("string", functionNode);
            functionNodes.add(suggestionInt);
            functionNodes.add(suggestionString);
        }

        return functionNodes;
    }

    public static DefinitionFunctionNode getPrimitivePrototype(String primitiveType, ICommonFunctionNode functionNode) {
        String prototype = createPrototypeWithSpecificParameter(primitiveType, functionNode);
    ...
    private static String createPrototypeWithSpecificParameter(String parameterReplacement, ICommonFunctionNode functionNode) {
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            String paramType = parameterReplacement;
            paramType = VariableTypeUtils.deleteStorageClasses(paramType);
...
//    }
            ...
            if (ast != null && ast instanceof CPPASTTemplateDeclaration) {
                ...
                for (ICPPASTTemplateParameter templateParameter : templateParameters)
                    if (templateParameter instanceof CPPASTSimpleTypeTemplateParameter) {
                        ...
                        if (!types.contains(templateType)) types.add(templateType);
                    }
            }
        }
        return types;
    ...
    public static String getTemplateRawName(String rawType) {
        ...
        String coreType = simpleRawType.replaceAll(IRegex.POINTER, SpecialCharacter.EMPTY).replaceAll(IRegex.ARRAY_INDEX, SpecialCharacter.EMPTY).replaceAll(IRegex.ADDRESS, SpecialCharacter.EMPTY);
    ...
    public static String getTemplateFullName(VariableNode variable) {
        ...
        List<INode> functions = Search.searchNodes(Environment.getInstance().getProjectNode(), new FunctionNodeCondition(), functionPath);

        if (functions.isEmpty())
            functions = Search.searchNodes(Environment.getInstance().getProjectNode(), new DefinitionFunctionNodeCondition(), functionPath);
        ...
        if (!functions.isEmpty()) {
            ...
            if (!possibleNodes.isEmpty()) {
                if (possibleNodes.size() == 1) fullName = Search.getScopeQualifier(possibleNodes.get(0));
            }
        }
...
//    }
    ...
    public static String[] getTemplateArguments(String rawType) {
        ...
        if (astNode instanceof IASTIdExpression) {
            ...
            if (fName instanceof ICPPASTQualifiedName) fName = fName.getLastName();
    ...
    public static String[] complexGetTemplateArguments(String rawType) {
        ...
        String rType = rawType.substring(rawType.indexOf(OPEN_TEMPLATE_ARG) + 1, rawType.lastIndexOf(CLOSE_TEMPLATE_ARG));
        ...
        for (String argument : tempTemplateArgs) {
            ...
            if ((!argument.contains(OPEN_TEMPLATE_ARG) && !argument.contains(CLOSE_TEMPLATE_ARG)) || (argument.contains(OPEN_TEMPLATE_ARG) && argument.contains(CLOSE_TEMPLATE_ARG) && Utils.countCharIn(argument, '<') == Utils.countCharIn(argument, '>'))) {
                templateArguments.add(argument);
                tempCompleteArg = "";
            } else {
                if (tempCompleteArg.isEmpty()) tempCompleteArg += argument;
                else tempCompleteArg += PARAMETER_SEPARATOR + argument;
    ...
    public static boolean isValidClass(ClassNode base, ClassNode derived, String[] baseArguments) {
        ...
        if (declSpec instanceof CPPASTCompositeTypeSpecifier) {
            ICPPASTCompositeTypeSpecifier.ICPPASTBaseSpecifier[] baseSpecs = ((CPPASTCompositeTypeSpecifier) declSpec).getBaseSpecifiers();
            ...
            for (ICPPASTCompositeTypeSpecifier.ICPPASTBaseSpecifier baseSpec : baseSpecs) {
                ...
                if (nameSpec instanceof ICPPASTTemplateId) {
                    ...
                    if (derivedName.equals(baseName)) {
                        List<String> templateParameters = Arrays.asList(Objects.requireNonNull(getTemplateParameters(derived)));

                        IASTNode[] templateArguments = ((ICPPASTTemplateId) nameSpec).getTemplateArguments();

                        if (templateArguments.length != baseArguments.length) return false;
...
//                            }
    ...
    public static String recursiveGetBaseClass(String type, ClassNode derived, final List<INode> bases) {
        if (bases == null || bases.isEmpty() || bases.get(0).equals(derived)) return type;
