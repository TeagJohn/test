...
public class StubManager {
    ...
    // private static String generateIteratorBody(ValueDataNode dataNode) throws
    // Exception {
    ...
    // source += "\n#endif\n";
    // } else {
    // source += String.format("return %s;",
    // iterator.getDataNode().getVituralName());
    // }
...
//                    source += withDeclaration.substring(withDeclaration.indexOf(SpecialCharacter.SPACE_STR) + 1);
    ...
    private static final String NEW_ITERATOR_TEMPLATE = "if (AKA_INTRA_FCOUNT == %d) {\n" +
            "%s\n" +
            "}\n";
        ...
        try {
            for (SubprogramNode child : stubSubprograms) {
                if (child != null && !(child instanceof IterationSubprogramNode)) {
                    ...
                    INode function = child.getFunctionNode();
                    String stubPath = getStubCodeFilePath(function.getAbsolutePath());

                    String testCaseStubCode = SpecialCharacter.EMPTY;

                    if (((SubprogramNode) child).isStub())
                        testCaseStubCode = generateStubCodeFor(testCaseName, (SubprogramNode) child);
    ...
    public static void initializeStubCode(String functionName, String unitName, String functionPath, String filePath) {
        ...
        logger.debug(
                "Initialize stub code for " + functionName + " in " + unitName + " to " + filePath + " successfully");
    }
    ...
    private static final String FILE_BANNER = "/** Static counting variable **/\n" +
