...
public class CTypeDependencyGeneration extends AbstractDependencyGeneration{
    ...
    private static final AkaLogger logger = AkaLogger.get(CTypeDependencyGeneration.class);
    ...
    private static List<Level> generateSpace(IVariableNode resolvedVarNode) {
        List<Level> levels = new ArrayList<>();
        ISourcecodeFileNode sourcecodeFileNode = Utils.getSourcecodeFile(resolvedVarNode);
        if (sourcecodeFileNode != null) {
            for (Dependency d : sourcecodeFileNode.getDependencies()) {
                if (d instanceof IncludeHeaderDependency && d.getEndArrow().equals(sourcecodeFileNode)) {
                    INode start = d.getStartArrow();
                    List<Level> ls = new VariableSearchingSpace(start).generateExtendSpaces();
                    levels.addAll(ls);
                }
            }
        }
    ...
    private INode performSystemSearch(IVariableNode resolvedVarNode) {
        ...
        conditions.add(new StructurevsTypedefCondition());

        // EXPERIMENTAL
        // List<INode> matches = Search.searchNodes(systemRoot, conditions.get(0))
        //         .stream()
    ...
    private INode performPrimitiveSearch(String name, String rawType) {
        ...
        if (VariableTypeUtils.isBasic(type) || VariableTypeUtils.isVoid(type)
                ...
                || VariableTypeUtils.isVoidMultiLevel(type)) {
    ...
    private INode performSimpleSearch(List<Level> spaces, IVariableNode resolvedVarNode) {
        ...
        if (VariableTypeUtils.isStructureMultiLevel(type)
            ...
            List<INode> allNodesInSpace = getCandidatesNodesInSpaces(spaces, null, searchedPath);
    ...
    private List<INode> getCandidatesNodesInSpaces(List<Level> spaces, List<SearchCondition> conditions, String searchSuffix) {
        ...
        List<INode> rt = new ArrayList<>(allNodesInSearchingSpace);
        return rt;
    }
    ...
    private INode performComplexSearch(List<Level> spaces, IVariableNode node) throws Exception {
        ...
        List<SearchCondition> conditions = new ArrayList<>();
        conditions.add(new ClassvsStructvsNamespaceCondition());

        String searchSuffix = File.separator + firstPrefix;

        // EXPERIMENTAL
        // List<INode> candidateNodes = Search.searchInSpace(spaces, conditions.get(0), searchSuffix);
        List<INode> candidateNodes = getCandidatesNodesInSpaces(spaces, conditions, searchSuffix);
    ...
    private INode performScopeSearch(List<Level> spaces, IVariableNode resolvedVarNode) {
        ...
        List<INode> possibles = getCandidatesNodesInSpaces(spaces, conditions, searchPath);
