...
//                ((TemplateSubprogramDataNode) node).setListArgOfTemplate(templateSubprogramDataNode.getListArgOfTemplate());
                for (IValueDataNode valueDataNode : templateSubprogramDataNode.getListArgOfTemplate()) {
                    ...
                    valueDataNode.setParent(node);
                }
                ((TemplateSubprogramDataNode) node).setRealTypeMapping(templateSubprogramDataNode.getRealTypeMapping());
            }
        ...
        if (node instanceof ArrayDataNode && node.getChildren().size() > 20) {
            ...
            for (int i = size - 1; i >= 0; i--) {
                IDataNode child = node.getChildren().get(i);
                if (child instanceof NormalDataNode && ((NormalDataNode) child).getValue() == null) {
                    node.getChildren().remove(i);
//                    logger.debug(node.getChildren().get(i).getName() + " is removed");
                }
            }
        }
    ...
    protected List<RandomValue> generateRandomValueForArguments(RootDataNode root, ICommonFunctionNode sut,
        ...
        //get
        List<IVariableNode> listVar = new ArrayList<>();
        if (selectedPrototype != null) {
            SubprogramNode sub = Search2.findSubprogramUnderTest(selectedPrototype.getRootDataNode());
            if (sub != null) {
                for (IDataNode node : sub.getChildren()) {
                    if (node instanceof ValueDataNode) {
                        VariableNode var = ((ValueDataNode) node).getCorrespondingVar();
    ...
    protected TestCase createTestcase(TestPrototype selectedPrototype, int iteration, ICommonFunctionNode functionNode) {
        return createTestcase(selectedPrototype, iteration, functionNode, ITestCase.POSTFIX_TESTCASE_BY_RANDOM);
    }
...
//            String newTestcaseName = nameofTestcase;
            ...
            if (testCase != null) {
                ...
                List<IDataNode> args = (subprogramNode == null) ? new ArrayList<>() : subprogramNode.getChildren();
                TreeExpander expander = new TreeExpander();
                for (IDataNode arg : args) {
                    if (arg instanceof ValueDataNode) {
                        try {
                            if (((ValueDataNode) arg).getCorrespondingVar() == null)
                                System.out.println();
                            expander.expandTree((ValueDataNode) arg);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
