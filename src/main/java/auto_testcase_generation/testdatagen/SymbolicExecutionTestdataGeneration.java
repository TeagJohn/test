    ...
    protected List<RandomValue> generateTheNextTestData(List<ICfgNode> testpath, ICommonFunctionNode functionNode,
        ...
        if (functionNode.isTemplate()) {
            ...
            for (int i = 0; i < listArgsVariables.size(); i++) {
                ...
                IVariableNode variableNode = listArgsVariables.get(i).clone();
                if (dataNode instanceof ValueDataNode) {
                    variableNode.setRawType(((ValueDataNode) dataNode).getRawType());
                    variableNode.setCoreType(((ValueDataNode) dataNode).getRealType());
                    variableNode.setReducedRawType(((ValueDataNode) dataNode).getRealType());
        ...
        try {
            ...
            if (functionNode.isTemplate()) {
                TemplateSubprogramDataNode templateSubprogramDataNode = (TemplateSubprogramDataNode) Search2.searchNodes(
                        allPrototypes.get(0).getRootDataNode(), TemplateSubprogramDataNode.class).get(0);
                String prototype = templateSubprogramDataNode.getRealFunctionNode().toString();
