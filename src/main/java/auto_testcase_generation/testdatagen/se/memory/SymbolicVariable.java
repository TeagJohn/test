...
import auto_testcase_generation.testdatagen.se.memory.basic.StringSymbolicVariable;
	...
	public static SymbolicVariable create(String name, String realType, INode nodeType, String defaultValue,
		...
		if (VariableTypes.isAuto(realType)) {
			...
			if (VariableTypeUtils.isNumBasic(realType)) {
				...
				} else if (VariableTypeUtils.isStrBasic(realType)) {
					v = new StringSymbolicVariable(name, realType, scopeLevel);
					((StringSymbolicVariable) v).getBlock().setName(defaultValue);
				} else {
					/*
					 * ----------------STRUCTURE----------------------
					 */
